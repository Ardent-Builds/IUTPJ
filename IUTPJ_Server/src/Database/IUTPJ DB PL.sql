...
---------------------------------SCHEMAS--------------------------
--------------Username: iutpj Password: IutPj---------------------

DROP TABLE CONTEST_PROBLEM_JUNCTION;
DROP Table CONTEST_SUBMISSION_JUNCTION;
...
CREATE TABLE CONTEST_SUBMISSION_JUNCTION(
    CONTEST_ID VARCHAR2(15) NOT NULL,
    SUBMISSION_ID VARCHAR2(15) NOT NULL,
    FOREIGN KEY(CONTEST_ID) REFERENCES CONTEST_INFO(ID) ON DELETE CASCADE,
...
------------------------ Contest_Info TRIGGERS -------------------------------
CREATE OR REPLACE TRIGGER UNLOCK_PROBLEM
AFTER UPDATE ON CONTEST_INFO
FOR EACH ROW
WHEN (NEW.FINISHED = 'OVER' AND OLD.FINISHED = 'RUNNING')
...
BEGIN
  ...
  INSERT INTO CONTEST_INFO VALUES(UNIQUE_ID, CNAME, STIME, DRTION, 'COMMING', SETTER);
  RETURN UNIQUE_ID;
...
BEGIN
  ...
  IF ISFINISHED = 'OVER' THEN RETURN 'OVER';
  END IF;

  UPDATE CONTEST_INFO
  SET CONTEST_NAME = CNAME, START_TIME = CSTIME, DURATION_MIN = DRTION, FINISHED = FINIS
...
CREATE OR REPLACE PROCEDURE UPDATE_STATE(
    NEXT_START_TIME OUT CONTEST_INFO.START_TIME%type,
    NEXT_FINISH_TIME OUT CONTEST_INFO.START_TIME%type
  )
AS
BEGIN

  UPDATE CONTEST_INFO
  SET FINISHED = 'RUNNING'
  WHERE FINISHED = 'COMMING' AND START_TIME <= SYSDATE;

  UPDATE CONTEST_INFO
  SET FINISHED = 'OVER'
  WHERE FINISHED = 'RUNNING' AND START_TIME + NUMTODSINTERVAL( DURATION_MIN, 'MINUTE') <= sysdate;

  NEXT_START_TIME := null;
  SELECT MIN(START_TIME) INTO NEXT_START_TIME
  FROM CONTEST_INFO
  WHERE FINISHED = 'COMMING';

  NEXT_FINISH_TIME := NULL;
  SELECT MIN(START_TIME + NUMTODSINTERVAL( DURATION_MIN, 'MINUTE')) INTO NEXT_FINISH_TIME
  FROM CONTEST_INFO
  WHERE FINISHED = 'RUNNING';
...
CREATE OR REPLACE FUNCTION SUBMISSION_FOR_CONTEST(      
  SUBMISSION_ID IN SUBMISSION.ID%TYPE, 
  CONTEST_ID IN CONTEST_INFO.ID%TYPE
  ) RETURN VARCHAR2
AS
  CONTEST_FINISHED  CONTEST_INFO.FINISHED%TYPE;
BEGIN
  ...
  IF CONTEST_FINISHED = 'OVER' THEN 
    RETURN 'OVER';
  END IF;
  INSERT INTO CONTEST_SUBMISSION_JUNCTION VALUES(CONTEST_ID, SUBMISSION_ID);
  RETURN 'PROBLEM ADDED';
END;
/
...
CREATE OR REPLACE FUNCTION CHANGE_PROBLEM_LOCK_STATE(
  PROBLEM_ID IN PROBLEM_SET.ID%TYPE,
  LOCKED_STATE IN PROBLEM_SET.LOCKED%TYPE
  )
  RETURN VARCHAR2
AS
BEGIN
  UPDATE PROBLEM_SET
  SET LOCKED = LOCKED_STATE
  WHERE ID = PROBLEM_ID;
  RETURN 'SUCCESS';
...
BEGIN
  UNIQUE_ID := TO_CHAR(SYSDATE,'DDMMYYYYHH24MISS');
  INSERT INTO SUBMISSION VALUES(UNIQUE_ID, PID, USID, CL, current_timestamp, VR, RT, CDF);
  RETURN UNIQUE_ID;
